...
import { Brain, Zap, RefreshCw, AlertTriangle, ChevronRight, Clock, BarChart2, Layers, Cpu, Network } from 'lucide-react';
...
const MODELS = [
  {
    ...
    name: 'Random Forest',
    icon: Layers,
    description: 'Ensemble de 200 árboles. Rápido, robusto, feature importance nativa.',
    tags: ['~5-10s', 'Robusto', 'Interpretable'],
    color: 'cyan',
  },
  {
    ...
    icon: Zap,
    description: 'Boosting iterativo. Mayor precisión que RF con patrones complejos.',
    tags: ['~15s', 'Alta precisión', 'Regularización'],
    color: 'purple',
  },
  {
    id: 'lstm',
    ...
    icon: Network,
    description: 'Red neuronal recurrente. Captura dependencias temporales largas con Monte Carlo Dropout.',
    tags: ['~60-90s', 'Deep Learning', 'MC Dropout CI'],
    color: 'green',
    badge: 'NEW',
  },
];
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-20 pb-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-5">
          ...
          <button onClick={handlePredict} disabled={loading || !ticker}
            ...
            {loading
              ...
              : <><Cpu className="w-4 h-4" /> Ejecutar predicción</>}
          </button>
        </div>
          ...
          {MODELS.map(m => {
            ...
            return (
              <button key={m.id} onClick={() => setModel(m.id)}
                ...
                {m.badge && (
                  <span className="absolute top-3 right-7 text-xs font-mono font-bold text-accent-green bg-accent-green/15 border border-accent-green/30 px-1.5 py-0.5 rounded">
                    {m.badge}
                  </span>
                )}
                <div className="flex items-start gap-3">
                  <div className={clsx('w-9 h-9 rounded-lg border flex items-center justify-center flex-shrink-0', c.icon)}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {m.tags.map(tag => (
                        <span key={tag} className={clsx('text-xs font-mono px-1.5 py-0.5 rounded bg-bg-elevated border border-border', active ? c.tag : 'text-text-muted')}>
                          {tag}
                        </span>
                      ))}
                    </div>
        ...
        <div className="bg-bg-card border border-border rounded-xl p-3 mb-5">
          <div className="flex flex-wrap items-center gap-1.5 text-xs font-mono text-text-secondary">
            {[
              model === 'lstm' ? '3 años OHLCV' : '3 años OHLCV',
        ...
        {error && (
          <div className="flex items-start gap-3 bg-accent-red/10 border border-accent-red/30 rounded-xl px-4 py-3 mb-5 text-sm font-mono text-accent-red">
            ...
            <div>
              ...
              {model === 'lstm' && <p className="text-xs text-text-muted mt-1">💡 El modelo LSTM requiere que el microservicio Python esté configurado y tenga TensorFlow instalado.</p>}
            </div>
          </div>
        )}
        ...
        {loading && (
          <div className="bg-bg-card border border-border rounded-2xl p-8 text-center mb-5">
            ...
            <p className="text-sm text-text-muted font-mono">
              {model === 'lstm'
                ? 'Descargando datos → Construyendo secuencias → Entrenando red LSTM → Monte Carlo Dropout…'
                : `Descargando 3 años → Calculando features → Entrenando ${model === 'random-forest' ? '200 árboles' : '150 estimadores'}…`}
            </p>
            {model === 'lstm' && (
              <p className="text-xs text-text-muted font-mono mt-2 opacity-60">El primer entrenamiento LSTM puede tardar 60-90 segundos</p>
            )}
          </div>
        )}
        ...
        {!mlData && !loading && !error && (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-text-muted font-mono max-w-md mx-auto">Selecciona ticker, modelo y horizonte temporal. El modelo se entrena en tiempo real con datos de Yahoo Finance.</p>
          </div>
        )}
        ...
        {mlData && !loading && (
          <div className="space-y-4">
            ...
            <div className="flex gap-1 bg-bg-card border border-border rounded-lg p-1 w-fit">
              {[
                { id: 'prediction' as const, label: '📈 Predicción', },
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={clsx('font-mono text-xs px-2 py-0.5 rounded border', colors.tag,
                        model === 'lstm' ? 'bg-accent-green/10 border-accent-green/25' :
                        model === 'gradient-boosting' ? 'bg-accent-purple/10 border-accent-purple/25' :
                    ...
                    <p className="text-xs text-text-muted font-mono">
                      {mlData.days_ahead} días hábiles · {new Date(mlData.last_updated).toLocaleString('es-ES')}
                    </p>
                  </div>
                  <button onClick={handlePredict}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-mono text-text-muted border border-border rounded-lg hover:text-text-primary hover:border-border-bright transition-all">
