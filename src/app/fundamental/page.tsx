...
export default function FundamentalPage() {
  ...
  const handleSubmit = (t: string) => { fetchData(t); router.replace(`/fundamental?ticker=${t.toUpperCase()}`); };
            ...
            <div className="bg-bg-card border border-border rounded-xl p-5 flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => router.push(`/technical?ticker=${data.ticker}`)}
                  className="p-1.5 border border-border text-text-muted hover:text-accent-cyan hover:border-accent-cyan/30 rounded-lg transition-all" title="Técnico">
                  <BarChart2 className="w-4 h-4" />
                </button>
                <button onClick={() => router.push(`/ml?ticker=${data.ticker}`)}
                  className="p-1.5 border border-border text-text-muted hover:text-accent-purple hover:border-accent-purple/30 rounded-lg transition-all" title="ML">
                  <Brain className="w-4 h-4" />
                </button>
                <button onClick={() => router.push(`/compare?tickers=${data.ticker}`)}
                  className="p-1.5 border border-border text-text-muted hover:text-accent-yellow hover:border-accent-yellow/30 rounded-lg transition-all" title="Comparar">
              ...
              <div className="xl:col-span-2 space-y-5">
                ...
                <Section title="Crecimiento y Dividendos">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 stagger">
                    ...
                    <MetricCard label="Dividend Yield"     value={m.dividend_yield     != null ? formatPercent(m.dividend_yield)         : '—'}   highlight={m.dividend_yield && (m.dividend_yield < 1 ? m.dividend_yield*100 : m.dividend_yield) > 1 ? 'cyan' : 'none'} description="Rentabilidad por dividendo" animationDelay={100} />
